  ...
  loadClass: function () {
    ...
    wx.request({
      ...
      data: {
        ...
        status: '0,1,2,3',  //状态，多状态以英文逗号分隔，0=等确认；1=待发班；2=已发班；3=已完成；
      },
      success: function (res) {
        ...
        if (res.data.resultdata) {
          ...
          for (var i = 0; i < temptaskList.length; i++) {
            ...
            //temptaskList[i].BeginTime = temptaskList[i].UseDate?temptaskList[i].BeginTime.substr(11, 8):'';
            //temptaskList[i].EndTime = temptaskList[i].EndTime?temptaskList[i].EndTime.substr(11, 8):'';

            temptaskList[i].UseDate = temptaskList[i].UseDate.substr(0, 10);
            temptaskList[i].BeginTime = temptaskList[i].BeginTime ? temptaskList[i].BeginTime.substr(11, 8) : '';
            temptaskList[i].EndTime = temptaskList[i].EndTime ? temptaskList[i].EndTime.substr(11, 8) : '';
          }
          that.setData({
