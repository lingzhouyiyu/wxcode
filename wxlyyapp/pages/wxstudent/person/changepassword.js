...
*/
  ...
  buttontap: function () {
    ...
    if (that.data.PrePswinputValue.length == 0 || that.data.NewPswinputValue.length == 0 || this.data.ConfimPswinputValue.length == 0){
      ...
      wx.request({
        url: app.XSDZhostUrl + 'AppTeachersManage/PublicInterface/SubmitResetPassword',
        method: 'GET',
        header: {
        ...
        data: {
          account: app.XSDZOBJ.Account,
          oldPassword: that.data.PrePswinputValue,
          password: that.data.NewPswinputValue
        },
